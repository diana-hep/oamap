...
PLUR shares many concepts with the ROOT file format, but there are differences. First, PLUR is not a data format: one could use ROOT as a source and storage for PLUR data structures (converting Numpy arrays to and from ROOT TBranches). But more significantly,
   ...
   2. ROOT materializes data as ordinary C++ objects so that C++ code can run on them. PLUR either uses Python's dynamism or instruments compiled code to provide the illusion that the user's code is operating on objects, when in fact it is operating on array indexes that produce the required values on demand.
   3. Data can be modified in-place with ROOT, but PLUR objects are immutable. Modifying a PLUR dataset, such as updating attribute values, creates a new column to be used in conjunction with the old columns like a diff-patch.
   4. ROOT schema evolution is complicated by the fact that C++ is a nominally typed language: classes must have the same name; it's not enough to have the right attributes. PLUR types are structurally typed, which allows old data to be read into new schemas.
...
Moreover, the [PLUR specification](../../wiki/Encoding-scheme) is language-neutral: it could be implemented in [Cling, a just-in-time C++ compiler](https://root.cern.ch/cling). The code transformations that provide the fastest access can be implemented more easily in C++ than in Python using template metaprogramming.
...
One important distinction between a real database and distributed PLUR is that databases maintain indexes over some fields for faster lookup than a sequential scan. PLUR can be used _with_ an indexed database if the database uses its fast index to produce an event list for PLUR to ingest.

An event list is a usually sorted, always unique list of pointers into a collection like `List(Event)`. The "pointers" themselves are just indexes in that collection (not raw memory pointers!). Event lists can be a very lightweight way to represent skims using one column instead of a copy of a subset of rows for every column of the original dataset.
...
To a user, this would require a combined query: an SQL part (for the database) followed by a Python part (for PLUR) to be executed on the selected entries. In the future, Femtocode could simplify this two-step process into a single expression.
